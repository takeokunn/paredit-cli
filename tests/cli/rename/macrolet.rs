...
#[test]
fn cli_writes_compiler_macrolet_rename_without_touching_expander_or_noncall_values() {
    let dir = fresh_temp_dir("rename-compiler-macrolet-write");
    let macrolet_file = dir.join("core.lisp");
    fs::write(
        &macrolet_file,
        "(compiler-macrolet ((old-name (x) (list old-name x))) (old-name 1) old-name)\n",
    )
    .expect("write compiler-macrolet fixture");
    ...
    assert_eq!(
        fs::read_to_string(&macrolet_file).expect("read rewritten compiler-macrolet fixture"),
        "(compiler-macrolet ((new-name (x) (list old-name x))) (new-name 1) old-name)\n"
    );
}
...
#[test]
fn cli_writes_macrolet_rename_without_crossing_labels_function_shadow() {
    let dir = fresh_temp_dir("rename-macrolet-labels-shadow-write");
    let macrolet_file = dir.join("core.lisp");
    fs::write(
        &macrolet_file,
        "(macrolet ((old-name (x) x)) (labels ((old-name (y) (old-name y))) (old-name 1)) (old-name 2))\n",
    )
    .expect("write labels shadow fixture");
    ...
    assert_eq!(
        fs::read_to_string(&macrolet_file).expect("read rewritten labels shadow fixture"),
