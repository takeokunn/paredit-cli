...
#[test]
fn cli_writes_replace_function_call_at_compiler_macrolet_expander_path() {
    let dir = fresh_temp_dir("replace-function-calls-compiler-macrolet-expander-path");
    let lisp_file = dir.join("service.lisp");
    fs::write(
        &lisp_file,
    ...
    assert_eq!(
        fs::read_to_string(lisp_file).expect("read rewritten lisp"),
        "(defun render () (compiler-macrolet ((expand (id) `(load-user ,id))) (expand user)))\n"
    );
}
