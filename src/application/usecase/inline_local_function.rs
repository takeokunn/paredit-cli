//! Use case for safely inlining a single Common Lisp `flet` call.
...
pub fn plan_inline_local_function(
    ...
    for (name, argument) in parameter_names.into_iter().zip(&call.children[1..]) {
        ...
        parameters.push(InlineLocalFunctionParameterPlan {
            name,
...
mod tests {
    ...
    fn plan(input: &str) -> Result<InlineLocalFunctionPlan> {
        plan_inline_local_function(InlineLocalFunctionRequest {
            input,
    ...
    #[test]
    fn rejects_dropped_or_duplicated_arguments() {
        assert!(plan("(flet ((f (x) 1)) (f (effect)))").is_err());
        assert!(plan("(flet ((f (x) (+ x x))) (f (effect)))").is_err());
    }

    #[test]
    fn rejects_multiple_bindings_labels_and_wrong_arity() {
        assert!(plan("(flet ((f (x) x) (g (x) x)) (f 1))").is_err());
        assert!(plan("(labels ((f (x) x)) (f 1))").is_err());
        assert!(plan("(flet ((f (x) x)) (f 1 2))").is_err());
    }
}
